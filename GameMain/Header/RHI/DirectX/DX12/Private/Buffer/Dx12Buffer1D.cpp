...
namespace XusoryEngine
{
	...
	void Dx12Buffer1D::CreateFixedBuffer(const Dx12Device* device, D3D12_RESOURCE_STATES initState, UINT64 size)
	{
	...
	void Dx12Buffer1D::CreateTex1DBuffer(const Dx12Device* device, D3D12_RESOURCE_STATES initState, UINT64 size, UINT16 arraySize, UINT16 mipLevels, DXGI_FORMAT format)
	{
		const CD3DX12_HEAP_PROPERTIES heapProperties(m_heapType);
		const CD3DX12_RESOURCE_DESC resourceDesc = CD3DX12_RESOURCE_DESC::Tex1D(format, size, arraySize, mipLevels);
